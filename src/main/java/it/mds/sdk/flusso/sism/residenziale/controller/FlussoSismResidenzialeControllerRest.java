...
package it.mds.sdk.flusso.sism.residenziale.controller;
...
@EnableAsync
@Slf4j
public class FlussoSismResidenzialeControllerRest implements ValidazioneController<RecordDtoSismResidenziale> {
    ...
    private final FlussoSismResService flussoSismResService;
    private final ConfigurazioneFlussoSismRes conf;

    private final MultiValueMap<String, String> headers;


    @Autowired
    public FlussoSismResidenzialeControllerRest(@Qualifier("parserRegoleSismRes") final ParserRegole parserRegole,
                                   @Qualifier("parserTracciatoSismRes") final ParserTracciato parserTracciato,
                                   @Qualifier("flussoSismResService") final FlussoSismResService flussoSismResService,
        ...
        this.flussoSismResService = flussoSismResService;
        this.conf = conf;
    ...
    @Override
    @PostMapping(path = "v1/flusso/sismresidenzialeprestazioni")
    public ResponseEntity<RisultatoInizioValidazione> validaTracciato(@RequestBody FlussoRequest flusso, String nomeFlussoController) {
        ...
        flussoSismResService.validazioneBlocchi(dimensioneBlocco, flusso.getNomeFile(), regoleFlusso, infoRun.getIdRun(),
    ...
    @Override
    @PostMapping("v1/flusso/sismresidenzialeprestazioni/record")
    public ResponseEntity<RisultatoValidazione> validaRecord(RecordRequest<RecordDtoSismResidenziale> recordRequest, String nomeFlusso) {
        return null;
    }

    @Override
    @GetMapping("v1/flusso/sismresidenzialeprestazioni/info")
